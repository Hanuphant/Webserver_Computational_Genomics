...
def trim(data_dir):
    ...
    if os.path.exists(os.path.join(args.o,'trimmed')):
        sys.exit('Please change the name of <data_dir> to something other than "trimmed"')
    else:
        print("Trimmed does not exist")
        os.makedirs(os.path.join(args.o,'trimmed'))
    if not os.path.exists(os.path.join(args.o,'fastqc')):
        print("fastqc does not exist")
        os.makedirs(os.path.join(args.o,'fastqc'))
    ...
    for root, subdirectories, files in os.walk(data_dir):
        for subdirectory in subdirectories:
            ...
            trim_dir = os.path.join(args.o,'trimmed', isolate_name)
            ...
            while True:
                ...
                fastqc_base = os.path.join(args.o,'fastqc', isolate_name)
    ...
    fastqc_path = os.path.join(args.o, 'fastqc')
    multiqc_path = os.path.join(args.o, 'multiqc')
    subprocess.call(shlex.split(f'multiqc {fastqc_path} -o {multiqc_path}'))

    print("\n\n\nDone trimming!\n\n\n")

def create_assemblies(trimmed_dir):
    if not os.path.exists(os.path.join(args.o,'spades_outputs')):
        os.makedirs(os.path.join(args.o,'spades_outputs'))
    ...
    for root, subdirectories, files in os.walk(trimmed_dir):
        for subdirectory in subdirectories:
            ...
            # Run SPAdes assmbly
            isolate_path = os.path.join(args.o, "spades_outputs", isolate_name)
            subprocess.call(shlex.split(f"spades.py --pe1-1 {trim_base}/{isolate_name}_1.fq --pe1-2 {trim_base}/{isolate_name}_2.fq -o {isolate_path}/"))
    ...
    for root, subdirectories, files in os.walk(spades_output_dir):
        for subdirectory in subdirectories:
            if 'CGT' in subdirectory:
                ...
                copy_path = os.path.join(job_output, isolate_name)
    ...
    for root, subdirectories, files in os.walk(assemblies_root):
        for subdirectory in subdirectories:
            if 'CGT' in subdirectory:
                ...
                isolate_base = os.path.join(assemblies_root, isolate_name)
...
if __name__ == "__main__":
    if not args.skip_trimming: # Trim reads before assembling
        trim(args.i)
        trimmed_dir = os.path.join(args.o, 'trimmed')
    else: # Skip trimming
        trimmed_dir = args.i.split('/')[0]

    # # Create assemblies
    create_assemblies(trimmed_dir)

    # Copy final assemblies to ./assemblies directory
    copy_final_assemblies(os.path.join(args.o,'spades_outputs'))

    # Run QUAST on final assemblies
    if not args.skip_quast:
        quast(args.o)

    os.system(f"rm -r {os.path.join(args.o, 'fastqc')}")
    os.system(f"rm -r {os.path.join(args.o, 'trimmed')}")
    os.system(f"rm -r {os.path.join(args.o, 'spades_outputs')}")
    os.system(f"rm -r {os.path.join(args.o, 'multiqc')}")
